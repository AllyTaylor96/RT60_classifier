...
audio_files = sorted(audio_filepath.glob('nick*.wav'))
...
def process_speech(speech_filepath, desired_sr, ir_waveform):
    ...
    ir_waveform = ir_waveform.numpy()
    padded_src_waveform = padded_src_waveform.numpy()
    convolved_speech_waveform = sg.fftconvolve(padded_src_waveform, ir_waveform)
    convolved_speech_waveform = torch.from_numpy(convolved_speech_waveform)
